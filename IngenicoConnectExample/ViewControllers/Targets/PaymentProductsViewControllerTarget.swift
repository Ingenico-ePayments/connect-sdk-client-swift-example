...
class PaymentProductsViewControllerTarget: NSObject, PKPaymentAuthorizationViewControllerDelegate, PaymentProductSelectionTarget, PaymentRequestTarget {
    ...
    func getBancontactJSON(success: (_ json: [String: Any])->Void, failure: (_ error: Error)->Void) {
        // *******************************************************************************
        //
                ...
                if paymentItem.identifier.isEqual(SDKConstants.kApplePayIdentifier) {
                    ...
                    if paymentProduct.identifier == AppConstants.kBancontactId {
                        self.getBancontactJSON(success:  { (json: [String:Any]) in
                            let vc = BancontactProductViewController(paymentItem: paymentProduct, session: self.session, context: self.context, viewFactory: self.viewFactory, accountOnFile: accountOnFile, customServerJSON: json)
    ...
    func show(paymentItem: PaymentItem, accountOnFile: AccountOnFile?) {
        ...
        if (AppConstants.kArvatoIds.contains(paymentItem.identifier)) {
            paymentProductForm = ArvatoProductViewController(paymentItem: paymentItem, session: session, context: context, viewFactory: viewFactory, accountOnFile: accountOnFile)
        }
        else if (paymentItem is PaymentProductGroup && paymentItem.identifier == "cards") || (paymentItem as? PaymentProduct)?.paymentMethod == "card" {
